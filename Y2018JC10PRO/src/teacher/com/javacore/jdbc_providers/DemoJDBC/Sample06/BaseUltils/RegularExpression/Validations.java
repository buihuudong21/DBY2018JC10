package teacher.com.javacore.jdbc_providers.DemoJDBC.Sample06.BaseUltils.RegularExpression;
...
public class Validations {
	...
	public static boolean isNumeric(String number) {
		...
		/*
		 * Number: A numeric value will have following format: ^[-+]?: Starts with an
		 * optional "+" or "-" sign. [0-9]*: May have one or more digits. \\.? : May
		 * contain an optional "." (decimal point) character. [0-9]+$ : ends with
		 * numeric digit.
		 */
	 ...
	 * ((?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{6,20}) Description ( # Start of
	 * group (?=.*\d) # must contains one digit from 0-9 (?=.*[a-z]) # must contains
	 * one lowercase characters (?=.*[A-Z]) # must contains one uppercase characters
	...
	public static boolean isSSNValid(String ssn) {
		boolean isValid = false;
		/*
		 * SSN format xxx-xx-xxxx, xxxxxxxxx, xxx-xxxxxx; xxxxx-xxxx: ^\\d{3}: Starts
		 * with three numeric digits. [- ]?: Followed by an optional "-" \\d{2}: Two
		 * numeric digits after the optional "-" [- ]?: May contain an optional second
		 * "-" character. \\d{4}: ends with four numeric digits.
		 * 
